...
def create_purchase(
    ...
    status, payload = idempotency_store.run(
        ...
        handler=lambda: (201, purchase_service.create(user, request, idempotency_key)),
    )
...
@router.post("/{purchase_id}/cancel", response_model=EnergyPurchase)
def cancel_purchase(purchase_id: str, user: AuthenticatedUser = Depends(get_current_user)) -> EnergyPurchase:
    return purchase_service.cancel(user, purchase_id)
